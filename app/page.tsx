...
export default function Home(){
 const [rows,setRows]=useState<Row[]>([]),[headers,setHeaders]=useState<string[]>([]),[qrCol,setQrCol]=useState(""),[labelPreset,setLabelPreset]=useState("11 × 11 cm Etikette"),[labelW,setLabelW]=useState(110),[labelH,setLabelH]=useState(110),[cols,setCols]=useState(1),[showName,setShowName]=useState(true),[nameCol,setNameCol]=useState(""),[showLocation,setShowLocation]=useState(true),[locationCol,setLocationCol]=useState(""),[qrSize,setQrSize]=useState(65),[qrTop,setQrTop]=useState(8),[titleSize,setTitleSize]=useState(9),[detailSize,setDetailSize]=useState(7.5),[textGap,setTextGap]=useState(6),[textAlign,setTextAlign]=useState<"left"|"center"|"right">("center"),[boldTitle,setBoldTitle]=useState(true),[busy,setBusy]=useState(false),[message,setMessage]=useState(""),[previewQr,setPreviewQr]=useState(""),[dragging,setDragging]=useState(false);
   ...
   try{
     ...
     for(let start=0;start<rows.length;start+=maxPerPage){
       ...
       for(let i=0;i<Math.min(maxPerPage,rows.length-start);i++){
         ...
         const qrX=x+(mm(labelW)-qr)/2,qrY=y+mm(labelH)-qr-mm(qrTop);
         page.drawImage(img,{x:qrX,y:qrY,width:qr,height:qr});
         let ty=qrY-mm(textGap)-titleSize;
         ...
         ty-=mm(6)+titleSize;
         if(showName&&nameCol){const t=String(row[nameCol]??"").slice(0,60);page.drawText(t,{x:textX(x,mm(labelW),textAlign,t,regular,detailSize),y:ty,size:detailSize,font:regular,maxWidth:mm(labelW)-mm(10)});ty-=mm(5)+detailSize}
 ...
 <label className="text-sm font-semibold">Abstand QR oben (mm)<input type="number" min="0" max="50" value={qrTop} onChange={e=>setQrTop(+e.target.value)} className="mt-2 w-full rounded-xl border p-3 font-normal"/></label>
 ...
 <div className="mt-5 rounded-2xl bg-slate-100 p-5"><div className="mx-auto overflow-hidden rounded-lg border bg-white p-3 shadow-sm" style={{width:"min(100%, 300px)",aspectRatio:labelW+"/"+labelH}}>{firstPreview&&qrCol?<div className={"flex h-full flex-col "+(textAlign==="center"?"items-center":textAlign==="right"?"items-end":"items-start")+" text-"+textAlign}>{previewQr?<img src={previewQr} alt="QR Vorschau" style={{width:Math.min(220,Math.max(50,qrSize*2.2)),marginTop:qrTop/2}} className="h-auto object-contain"/>:<div className="text-sm text-slate-400">QR wird geladen …</div>}<div className="w-full break-words text-sm" style={{marginTop:textGap*2.834645669/2,fontSize:titleSize,fontWeight:boldTitle?700:400}}>{String(firstPreview[qrCol]??"")}</div>{showName&&nameCol&&<div className="w-full break-words text-slate-600" style={{fontSize:detailSize}}>{String(firstPreview[nameCol]??"")}</div>}{showLocation&&locationCol&&<div className="w-full break-words text-slate-500" style={{fontSize:detailSize}}>{String(firstPreview[locationCol]??"")}</div>}</div>:<div className="flex h-full items-center justify-center text-sm text-slate-400">Excel-Datei hochladen</div>}</div></div>
