...
export default function Home(){
 const [rows,setRows]=useState<Row[]>([]),[headers,setHeaders]=useState<string[]>([]),[qrCol,setQrCol]=useState(""),[labelPreset,setLabelPreset]=useState("11 × 11 cm Etikette"),[labelW,setLabelW]=useState(110),[labelH,setLabelH]=useState(110),[cols,setCols]=useState(1),[showName,setShowName]=useState(true),[nameCol,setNameCol]=useState(""),[showLocation,setShowLocation]=useState(true),[locationCol,setLocationCol]=useState(""),[qrSize,setQrSize]=useState(65),[qrTop,setQrTop]=useState(8),[qrX,setQrX]=useState(0),[qrY,setQrY]=useState(0),[textXOffset,setTextXOffset]=useState(0),[textYOffset,setTextYOffset]=useState(0),[titleSize,setTitleSize]=useState(9),[detailSize,setDetailSize]=useState(7.5),[textGap,setTextGap]=useState(6),[textAlign,setTextAlign]=useState<"left"|"center"|"right">("center"),[boldTitle,setBoldTitle]=useState(true),[busy,setBusy]=useState(false),[message,setMessage]=useState(""),[previewQr,setPreviewQr]=useState(""),[dragging,setDragging]=useState(false);
   ...
   try{
     ...
     for(let start=0;start<rows.length;start+=maxPerPage){
       ...
       for(let i=0;i<Math.min(maxPerPage,rows.length-start);i++){
         ...
         const qrX=x+(mm(labelW)-qr)/2+mm(qrX),qrY=y+mm(labelH)-qr-mm(qrTop)+mm(qrY);
 ...
 <label className="text-sm font-semibold">QR links/rechts (mm)<input type="number" min="-50" max="50" value={qrX} onChange={e=>setQrX(+e.target.value)} className="mt-2 w-full rounded-xl border p-3 font-normal"/><span className="mt-1 block text-xs font-normal text-slate-500">− links · + rechts</span></label>
 <label className="text-sm font-semibold">QR vertikal (mm)<input type="number" min="-50" max="50" value={qrY} onChange={e=>setQrY(+e.target.value)} className="mt-2 w-full rounded-xl border p-3 font-normal"/><span className="mt-1 block text-xs font-normal text-slate-500">− oben · + unten</span></label>
