...
const presets=[
 {name:"11 × 11 cm Etikette",w:110,h:110},
 {name:"10 × 10 cm Etikette",w:100,h:100},
 {name:"10 × 5 cm Etikette",w:100,h:50},
 {name:"7 × 5 cm Etikette",w:70,h:50},
 {name:"A4",w:210,h:297},
 {name:"Benutzerdefiniert",w:70,h:55},
];
...
export default function Home(){
 ...
 async function makePdf(){
   ...
   const pageW=mm(210),pageH=mm(297),margin=mm(10),gap=mm(4);
   ...
   try{
     ...
     for(let start=0;start<rows.length;start+=maxPerPage){
       ...
       for(let i=0;i<Math.min(maxPerPage,rows.length-start);i++){
         const row=rows[start+i],c=i%perRow,r=Math.floor(i/perRow),x=margin+c*(mm(labelW)+gap),y=pageH-margin-(r+1)*mm(labelH)-r*gap,value=String(row[qrCol]??"");
         if(!value)continue;
         const png=await QRCode.toDataURL(value,{width:500,margin:1,errorCorrectionLevel:"M"}),img=await pdf.embedPng(png),qr=mm(Math.min(labelW-10,labelH-22));
         page.drawRectangle({x,y,width:mm(labelW),height:mm(labelH),borderColor:rgb(.78,.84,.87),borderWidth:.7});
         const qrX=x+(mm(labelW)-qr)/2,qrY=y+mm(labelH)-qr-mm(8);
         page.drawImage(img,{x:qrX,y:qrY,width:qr,height:qr});
         let ty=qrY-mm(6);
         page.drawText(value.slice(0,35),{x:x+mm(5),y:ty,size:9,font,maxWidth:mm(labelW)-mm(10)});
         ty-=mm(6);
         if(showName&&nameCol){page.drawText(String(row[nameCol]??"").slice(0,35),{x:x+mm(5),y:ty,size:7.5,font,maxWidth:mm(labelW)-mm(10)});ty-=mm(5)}
         if(showLocation&&locationCol)page.drawText(String(row[locationCol]??"").slice(0,35),{x:x+mm(5),y:ty,size:7.5,font,maxWidth:mm(labelW)-mm(10)})
       }
     }
     const bytes=await pdf.save(),safeBytes=new Uint8Array(bytes),blob=new Blob([safeBytes.buffer],{type:"application/pdf"}),url=URL.createObjectURL(blob),a=document.createElement("a");
     a.href=url;a.download="QR-Generator47-Etiketten.pdf";a.click();URL.revokeObjectURL(url);setMessage("PDF fertig – "+rows.length+" QR-Etiketten.")
   }catch(e){console.error(e);setMessage("PDF-Erstellung fehlgeschlagen.")}finally{setBusy(false)}
 }

 return <main className="min-h-screen"><header className="border-b bg-white"><div className="mx-auto max-w-6xl px-6 py-6"><div className="text-2xl font-bold">QR Generator<span className="text-sky-600">47</span></div><div className="text-sm text-slate-500">Excel → QR-Codes → druckfertiges PDF</div></div></header>
 ...
 {rows.length>0&&<div className="card p-7"><h2 className="text-xl font-bold">Daten & Etikett</h2><div className="mt-5 grid gap-4 sm:grid-cols-2">
 ...
 <div className="mt-5 rounded-2xl bg-slate-100 p-5"><div className="mx-auto overflow-hidden rounded-lg border bg-white p-3 shadow-sm" style={{width:"min(100%, 300px)",aspectRatio:labelW+"/"+labelH}}>{firstPreview&&qrCol?<div className="flex h-full flex-col items-center justify-center text-center">{previewQr?<img src={previewQr} alt="QR Vorschau" className="h-auto max-h-[62%] max-w-[72%] object-contain"/>:<div className="text-sm text-slate-400">QR wird geladen …</div>}<div className="mt-2 w-full break-words text-sm font-bold">{String(firstPreview[qrCol]??"")}</div>{showName&&nameCol&&<div className="w-full truncate text-xs text-slate-600">{String(firstPreview[nameCol]??"")}</div>}{showLocation&&locationCol&&<div className="w-full truncate text-xs text-slate-500">{String(firstPreview[locationCol]??"")}</div>}</div>:<div className="flex h-full items-center justify-center text-sm text-slate-400">Excel-Datei hochladen</div>}</div></div>
