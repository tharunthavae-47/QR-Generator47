...
export default function Home(){
 ...
 async function printLabels(){
   ...
   try{
     ...
     printWindow.document.write("<!doctype html><html><head><title>QR Generator47 – Etiketten</title><style>@page{size:A4;margin:0}*{box-sizing:border-box}html,body{margin:0;padding:0;background:#fff}body{font-family:Arial,sans-serif}.sheet{width:190mm;margin:10mm auto 0;display:grid;grid-template-columns:repeat("+Math.max(1,cols)+","+labelW+"mm);gap:4mm;align-content:start}.label{position:relative;overflow:hidden;border:.2mm solid #c7d2da;page-break-inside:avoid}.qr{position:absolute;object-fit:contain}.texts{position:absolute;left:5mm;right:5mm;top:0;line-height:1.15}.title,.detail{overflow-wrap:anywhere}.detail{margin-top:2mm}@media print{body{margin:0}.label{border-color:#d0d8dd}}</style></head><body><div class=\"sheet\">"+items.join("")+"</div><script>window.onload=function(){setTimeout(function(){window.focus();window.print()},300)}<\/script></body></html>");
